...
String ProjectName = "Progetto MC1   0.84";
int actualpage = -1;
...
void loop()
{
    ...
    if (myTouch.dataAvailable())
    {
      ...
      // MOVE MENU
      if ((y>=30) && (y<=80))  // Upper row
      {
        if ((x>=10) && (x<=150))  // Button: Left
        {
          waitForIt(10, 30, 150, 80);
          Serial.println("--LEFT");
        }
        if ((x>=160) && (x<=300))  // Button: Right
        {
          waitForIt(160, 30, 300, 80);
    ...
    if (stringComplete) {
      ...
      if (inputString.startsWith("?")) Serial.println("display");
...
// Draw the two buttons FOR MOVE
  myGLCD.setColor(0, 0, 255);
  myGLCD.fillRoundRect (10, 30, 150, 80);
  myGLCD.setColor(255, 255, 255);
  myGLCD.drawRoundRect (10, 30, 150, 80);
  myGLCD.print("LEFT", 50, 47);
  myGLCD.setColor(0, 0, 255);
  myGLCD.fillRoundRect (160, 30, 300, 80);
  myGLCD.setColor(255, 255, 255);
  myGLCD.drawRoundRect (160, 30, 300, 80);
  myGLCD.print("RIGHT", 195, 47);
  myGLCD.setBackColor (0, 0, 0);
